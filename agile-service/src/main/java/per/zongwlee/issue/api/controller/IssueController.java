...
public class IssueController extends BaseController {
    ...
    public ResponseEntity<Page<IssueDTO>> queryBacklogs(
            @ApiParam(value = "分页参数") PageRequest pageRequest) {
        return Optional.ofNullable(issueService.pageQueryBacklog(pageRequest))
                .map(result -> new ResponseEntity<>(result, HttpStatus.OK))
    ...
    public ResponseEntity<Page<IssueDTO>> queryActiveMatters(
            @ApiParam(value = "分页参数") PageRequest pageRequest) {
        return Optional.ofNullable(issueService.pageQueryActiveMatters(pageRequest))
                .map(result -> new ResponseEntity<>(result, HttpStatus.OK))
    ...
    public ResponseEntity<Page<IssueDTO>> queryFinishedMatters(
            @ApiParam(value = "分页参数") PageRequest pageRequest) {
        return Optional.ofNullable(issueService.pageQueryFinishedMatters(pageRequest))
                .map(result -> new ResponseEntity<>(result, HttpStatus.OK))
